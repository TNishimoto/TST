...
namespace tst
{
	...
	*/
	...
	void TST::getPathString(NodeIndex index, istring &result)
	{
		...
		NodeIndex now = index;
		TSTNode* node = &(*this)[index];
		if(index.first == TSTNode::BOTTOM) return;
		while (!node->isRoot())
		{
			vector<ichar> str;
	...
	void TST::construct(istring &text, uint64_t qgram)
	{
		...
		while (i < text.size()) {			
			...
			if (i + (newEdgeLength - 1) >= text.size()) break;
			...
			if (b)
			{
				...
				if (i % (PROCESSBAR2) == 0)
					{
						std::cout << "\r processing : " << "[" << (i/PROCESSBAR2) << "/" << (text.size() / PROCESSBAR2) << "]"  << std::flush;
					}
			}
		}
		std::cout << std::endl;
...
void TST::load(ifstream& ifs){
			...
			if(this->childrens.size() == 0) throw "error";
	...
	void TST::constructQgramTree(vector<vector<uint64_t>>& _childrens, vector<ichar>& _labels, vector<uint64_t>& qgramNodePositionsOfTSTLeave){
        ...
        while (leafIndexStack.size() > 0)
        {
            ...
            leafIndexStack.pop();
            //leafIndex;
            if(this->nextLeave[leafIndex] == TSTNode::EMPTY) continue;
...
bool TST::translatePattern(istring& pattern, vector<uint64_t>& result, bool isText){
		result.clear();
		if(isText)result.resize(pattern.size() - this->truncatedLength + 1);

		TSTNodePointer pointer(*this, ROOTINDEX, 0);
		bool b = false;
		if((pattern.size() > PROCESSBAR2))std::cout << std::endl;
		for(uint64_t i = 0;i<pattern.size();i++){
			if ((i+1) % (PROCESSBAR2) == 0)
			{
				std::cout << "\r processing : " << "[" << (i/PROCESSBAR2) << "/" << (pattern.size() / PROCESSBAR2) << "]"  << std::flush;
			}
			...
			if(pointer.isOnLeaf()){
				...
				if(i + 1 != pattern.size())pointer.moveNextLeave();				
			}
		}
		if((pattern.size() > PROCESSBAR2))std::cout << std::endl;
		return true;
	...
	std::pair<bool, uint64_t> TST::count(istring& pattern){
		TSTNodePointer pointer(*this, ROOTINDEX, 0);
		bool b = false;
		for(auto it : pattern){
			b = pointer.proceed(it);
	...
	void TST::constructCountNonLeaveVec(const vector<uint64_t>& translatedText){
		...
		for(auto it : translatedText) countLeaveVec[it]++;
		for(auto it : countLeaveVec) if(it == 0) throw "error";

		this->countNonLeaveVec.resize(this->internalNodes.size());
		for(uint64_t i=0;i<this->countNonLeaveVec.size();i++) this->countNonLeaveVec[i] = UINT64_MAX;
		...
		while(notCountedNonLeaveStack.size() > 0){
			...
			if(checkChildrenCounted(top)){
				...
				for(auto child : this->childrens[top]){
					if(!child.second) notCountedNonLeaveStack.push(child.first);
				}
			}
		}
